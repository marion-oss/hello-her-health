/**
 * anoqi — Retrieval-Augmented Generation: query-time retrieval
 *
 * Embeds the user's message, runs hybrid search over rag_chunks
 * (cosine similarity + full-text), merges with Reciprocal Rank Fusion,
 * returns the top-K snippets that get cited by the LLM.
 *
 * Called by api/functions/chat.ts before the Gemini chat call.
 *
 * Failure mode: if embedding or DB calls fail, returns []. Chat continues
 * without snippets (the LLM falls back to its base behavior). Retrieval
...
// Journeys without a matching pathway (menopause, fertility, free_chat)
// fall through to language-only retrieval.
...
export interface RawHit {
  ...
  similarity?:      number   // vector RPC
  rank?:            number   // FTS RPC
}

export interface Snippet {
  /** Stable label assigned at retrieve-time. The LLM cites snippets by this label, e.g. [S1]. */
  label:            string                  // 'S1' | 'S2' | ...
  rowId:            string                  // rag_chunks.id
  sourceKind:       SourceKind
  sourceRef:        string                  // pathway/source/red-flag id
...
// ─────────────────────────────────────────────────────────────
/**
 * Get a 768-dim embedding for a query string. Returns null on any failure
 * — caller should treat this as "skip retrieval, continue chat".
 */
  ...
  try {
    const res = await fetch(GEMINI_EMBED_URL, {
      ...
      body: JSON.stringify({
        ...
        // task_type RETRIEVAL_QUERY is the recommended setting for query-side
        // embeddings against a RETRIEVAL_DOCUMENT-embedded corpus.
...
export async function retrieve(
  ...
  const overfetch   = topK * 3   // each ranker overfetches; RRF needs depth

  const queryVec = await embedQuery(opts.message)

  // Fire vector + FTS in parallel. Either may fail; retrieval degrades
  // to whichever side returned data. Both failing → empty list.
...
// ─────────────────────────────────────────────────────────────
/**
 * Reciprocal Rank Fusion. Each ranker contributes 1 / (RRF_K + rank).
 * Items appearing in both rankers naturally rise to the top.
 */
...
// ─────────────────────────────────────────────────────────────
/**
 * Render snippets into the French addendum that gets appended to the
 * Anoqi system prompt. The LLM is instructed to cite by [Sk] label.
 *
 * If snippets is empty, returns null — caller should not append anything.
 */
